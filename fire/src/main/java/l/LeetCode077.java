...
public class LeetCode077 {
    ...
    public static void main(String[] args) {
        LeetCode077 leetCode077 = new LeetCode077();
        leetCode077.combine(4,2);
        for (List e: result){
    ...
    public void combine(int n, int k) {
        ...
        for (int i = 1; i <= n; i++) {
            combineSet.add(i);
            combine(n,i+1,k-1,combineSet);
            combineSet.remove(i);
        }
    }

    private void combine(int n, int from,int i, Set<Integer> combineSet) {
        if(i ==0){
        ...
        for (int j = from; j <= n; j++) {
            combineSet.add(j);
            combine(n,from+1,i-1,combineSet);
            combineSet.remove(j);
        }
    }
}
