...
1 <= k <= n!
    ...
    * */
     ...
     * <p>
     * int from = k/(n-1)! 为 (n-1)!的个数，index = from+1 ：
     * example: 第一位的值 k=15 ,15/(4-1)!=15/6 = 2 ，第一个元素为 2+1 =3
     * 剩余的 k 值为： k- from * (n-1)!
     * example: 剩余的 k 值为 15 - 2*6 = 3
     * 第二个元素：from = k/(n-2)! 为 (n-2)!的个数，index = from+1
     * example： 3/2! = 1, 经过第一轮，挑剩下的是 1，2，4；index = from+1,第二个元素就是 2
     * 第三个元素： 剩下的 k 值：k- from * (n-2)! 相对应的是 3- 1*(2!) = 1,    from = k/(n-3)! 为 (n-3)!的个数，
     * example  1/1! = 1 , 经过第一轮，挑剩下的是 1，4；
     */
